...
(function () {
  ...
  /* New-format publishable keys (sb_publishable_…) are NOT JWTs. Sending them
     as "Authorization: Bearer" makes the gateway try to parse a JWT and reject
     the request with 401, so the key goes in the apikey header alone.
     Legacy anon keys (eyJ… JWTs) still need the Bearer header. */
  var IS_JWT = KEY.indexOf('ey') === 0;
